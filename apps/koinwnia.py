...
def update_figure(figure, title):
    ...
    margin=dict(l=30, r=30, t=150, b=70))
...
fig= px.bar(erwtisi_6, x='erwtisi', y='timi', labels = {"apantisi": "", "timi":""}, 
               ...
               facet_col='vathmos', facet_row_spacing=0.08, height=500)
# remove "=" from layout
...
fig_3.update_layout(hovermode='y', 
                    legend=dict(
    yanchor="top",
    y=1.05,
    xanchor="center",
...
fig_4 = px.bar(erwtisi_19, x='timi', y='apantisi', color='vathmos', labels = {"apantisi": "", "timi":""}, 
               ...
               facet_col='date', height=500, width=1000)
# remove "=" from layout
for a in fig_4.layout.annotations:
    a.text = a.text.split("=")[1]
# automated figure update 
update_figure(fig_4, 'Ποιες θα είναι οι μακροχρόνιες επιπτώσεις της πανδημίας; Θα επηρεάσει θετικά, ουδέτερα ή αρνητικά...')
#specific legend position
fig_4.update_layout(hovermode='y', legend=dict(
    yanchor="top",
    y=1.16,
    xanchor="center",
    x=0.47))
...
update_figure(fig_5, 'Σκέφτεστε να εμβολιαστείτε όταν θα είναι διαθέσιμο δωρεάν ένα εμβόλιο<br>κατά του νέου κορωνοϊού, εγκεκριμένο από<br>την Ε.Ε. και τις υγειονομικές αρχές της χώρας μας;')
...
update_figure(fig_6, 'Με ποια από τις παρακάτω προτάσεις συμφωνείτε περισσότερο;<br>(N=66.3%, Σίγουρα/Μάλλον Ναι)')
# smaller text in graph
...
for a in fig_7.layout.annotations:
    a.text = a.text.split("=")[1]
# automated figure update
update_figure(fig_7, 'Πόσο σημαντική είναι καθεμία από τις παρακάτω απειλές που<br>αντιμετωπίζει σήμερα η χώρα μας;<span style="font-size: 13px;"><br>(0=καθόλου σημαντική έως 10=πάρα πολύ σημαντική)</span>')
#specific legend position
fig_7.update_layout(hovermode='y',legend=dict(
    yanchor="top",
    y=1.10,
    xanchor="center",
    x=0.47))
...
fig_8 = px.bar(erwtisi_7, x='date', y='timi',  labels = {"timi": "", 'date': ""}, 
               ...
               facet_col='apantisi', facet_col_wrap=4)
...
#specific legend position
fig_9.update_layout(hovermode='y',legend=dict(
    yanchor="top",
    y=1.10,
    xanchor="center",
    x=0.47))
...
#automated figure update
update_figure(fig_10, 'Eίναι ικανοποιητικός ο ρυθμός εμβολιασμών μέχρι τώρα ή όχι; (NUTS 1)')
#specific legend position
...
# automated figure update
update_figure(fig_11, 'Η χώρα μας αυτή την περίοδο κινείται προς τη σωστή ή προς τη λάθος κατεύθυνση;')
...
# Figure 12
fig_12 = px.bar(erwtisi_2, x='value', y='Unnamed: 0', color='date', labels = {"value": "", 'Unnamed: 0': ""}, 
               color_discrete_sequence=px.colors.qualitative.Bold, facet_col_wrap=2, height=1000,
               facet_col='date', text='value')
# remove "=" from layout
for a in fig_12.layout.annotations:
    a.text = a.text.split("=")[1]
#automated figure update
update_figure(fig_12, 'Ποια συναισθήματα σας διακατέχουν πιο έντονα<br>σήμερα ως Έλληνα/Ελληνίδα;<span style="font-size: 12px;"> (πρώτη αναφορά)</span>')
#specific legend position
fig_12.update_layout(hovermode='y',legend=dict(
    yanchor="top",
    y=1.06,
    xanchor="center",
    x=0.47))
...
# Figure 14
fig_14 = px.bar(erwtisi_19, x='date', y='timi', color='apantisi', labels = {"apantisi" : "", "timi" : ""}, 
               color_discrete_sequence=px.colors.qualitative.T10, barmode='group', text='timi')
# automated figure update
...
# Figure 15
fig_15 = px.bar(erwtisi_32, x='date', y='timi', color='apantisi', labels = {"timi": "", 'apantisi': 'Απάντηση'}, 
               color_discrete_sequence=px.colors.qualitative.Safe, text='timi', facet_col_wrap=2,
               facet_col='apantisi', height=900)
# remove "=" from layout
...
layout = html.Div(children=[
    html.Br(),
    dcc.Markdown('**ΟΙΚΟΝΟΜΙΚΕΣ ΕΞΕΛΙΞΕΙΣ & ΚΟΙΝΩΝΙΚΕΣ ΕΠΙΠΤΩΣΕΙΣ ΤΗΣ ΠΑΝΔΗΜΙΑΣ COVID-19 ΣΤΗΝ ΕΛΛΑΔΑ**', # bold text
    ...
    html.Div([
        html.Div(children=[
            html.H4('ΚΟΙΝΩΝΙΚΕΣ ΕΠΙΠΤΩΣΕΙΣ & ΑΠΟΨΕΙΣ', style={'font-family': 'arial', 'fontSize': 22, 'textAlign': 'center' }),
            html.Br(),
            dcc.Textarea(value='Επιλεγμένες ερωτήσεις 6 ερευνών του οργανισμού έρευνας και ανάλυσης «διαΝΕΟσις». Οι έρευνες αυτές διεξήχθησαν τον Απρίλιο, Σεπτέμβριο και Δεκέμβριο 2020, και Ιανουάριο, Μάρτιο και Μάιο 2021, οι οποίες απαρτίζονται από ερωτήσεις με κεντρικό άξονα την πανδημία. Κατά μέσο όρο, σε κάθε έρευνα έλαβαν μέρος 1.153 άτομα ηλικίας 17 ετών και άνω.', style={'font-family': 'arial', 'width': '100%', 'height': 50, 'textAlign': 'center', 'border':'none', 'resize': 'none'}),
            html.Br(),
            html.Br(),
            ...
            dcc.Graph(figure=fig_9, style= {'display':'inline-block', 'height': '100%', 'width': '50%'}),
            dcc.Graph(figure=fig_10, style= {'display':'inline-block', 'height': '100%', 'width': '50%', 'vertical-align': 'top'}),
            html.H6('Λήξη δεύτερου lockdown - Μάιος 2021', style={'font-family': 'arial', 'textAlign': 'center', 'text-decoration': 'underline'}),
            dcc.Graph(figure=fig_11, style= {'display':'inline-block', 'height': '100%', 'width': '50%'}),
            dcc.Graph(figure=fig_13, style= {'display':'inline-block', 'height': '100%', 'width': '50%'}),
            dcc.Graph(figure=fig_15, style= {'display':'inline-block', 'vertical-align': 'top', 'height': '100%', 'width': '50%'}),
            dcc.Graph(figure=fig_12, style= {'display':'inline-block', 'height': '100%', 'width': '50%'}),
            dcc.Graph(figure=fig_14, style= {'height': '80%', 'width': '50%', 'margin': '0 auto'}),
